...
class Review(models.Model):
    ...
    year_month = models.CharField("연월", max_length=7, help_text="예: 2025-02")
    ...
    def __str__(self):
        return f"{self.store.name} - {self.rating}점 ({self.year_month})"


class MonthlySummary(models.Model):
    store = models.ForeignKey(
        Store, on_delete=models.CASCADE, related_name="monthly_summaries"
    )
    year_month = models.CharField("연월", max_length=7)
    summary = models.TextField("AI 요약", blank=True)
    highlights = models.JSONField("하이라이트", default=dict, blank=True)
    avg_rating = models.FloatField("월 평균 평점", default=0.0)
    review_count = models.IntegerField("월 리뷰 수", default=0)
    ...
    def __str__(self):
        return f"{self.store.name} - {self.year_month} 요약"
